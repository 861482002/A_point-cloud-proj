...
class Benchmark(nn.Module):
    ...
    def forward(self, x, y):
        x=x.to(torch.float32)                           # x是输入的点云，y是目标点云(就是x往y上面配准)
        x_f, y_f = self.encoder(x), self.encoder(y)     # 获取点云特征
...
class IterativeBenchmark(nn.Module):
    ...
    def forward(self, x, y):
        ...
        ----------
        x: [1,3,2048]   待配准的点云
        y: [1,3,2048]   目标点云（需要将x往y上面配准）

        Returns
        -------
        transformed_xs:list  我们需要取最后一个
        '''
        transformed_xs = []
        device = x.device
        B = x.size()[0]
    ...
    def only_reg(self, x, y):
        ...
        return transformed_xs[-1]


if __name__ == '__main__':
    x, y = torch.randn(4, 3, 5), torch.randn(4, 3, 5)
    net = IterativeBenchmark(in_dim=1, niters=2,gn=True)
    # print(net)
